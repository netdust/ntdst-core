...
- A verb outside the READ allow-list (`GET`, `HEAD`, `OPTIONS`) with no capability **does not register** (FR-4, revision 3).
...
final class NTDST_Rest {
    ...
    //   callable      → as given
    // registerOne() refuses any verb OUTSIDE the READ allow-list (GET|HEAD|OPTIONS) whose resolved
    // permission is a shorthand posture — an allow-list, so a custom verb (PURGE) is not read as safe (gate C2).
    // defaults() may set a posture ('logged_in', a capability) but never an opening ('public' or a callable):
    // an opening default is refused (defaults:opening) and dropped, and the defaults a route inherits are
    // SNAPSHOT when its verb runs, so a later defaults() cannot reach back (gate C2).
    public function cors(array|callable $origins, bool $credentials = false, int $maxAge = 0): self; // T06 — feeds allowed_http_origins, REST requests only (row #9)
    public static function corsDecision(?string $origin, array $policy): array;  // unchanged signature; $policy carries NO origins — allowed-ness is asked of is_allowed_http_origin()
