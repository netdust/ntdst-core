...
- **SC-3:** `grep -rn "register_rest_route(\|wp_ajax_\|ntdst/api_data\|ntdst_actions\|get_nonce\|ntdstAPI" --include=*.php --include=*.js . | grep -vE "^(\./)?api/Rest\.php|/vendor/|/tests/|README"` returns 0 lines *(form hardened at the freshness review — ugrep prints no `./` prefix)*; `assets/js/ntdst-api.js` and `api/Actions.php` do not exist.
