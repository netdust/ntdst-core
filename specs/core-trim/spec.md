...
always be loaded using composer"). Carries 2 `[NEEDS CLARIFICATION]` markers for
Stefan; the gate halts on them by design. Awaiting review, then `writing-plans`.
...
- **FR-2:** The per-service enable switch — option `ntdst_service_{slug}`, filter `ntdst_service_{slug}_enabled`, `getServiceSlug()`, `declaredServiceName()`, `slugCache` — is removed. A service is off when `metadata()['enabled'] === false` or when its `conditional` entry's condition returns false; there is no third way. The fail-open wart named in `docs/philosophy.md` §4 is gone with it. `services.overrides` and `ntdst_service_{slug}_config` keep working only if the plan's ground-truth finds stride's one `overrides` entry read by a service; otherwise they go too (plan task). `getServiceConfig()`, `getServices()`, `getBootedServices()`, `hasService()`, `isBooted()` are removed. `register()`, `bootCore()`, `bootFeatures()`, `config()` and the three lifecycle hooks stay as they are.
...
- **FR-8:** `NTDST_Theme` loses `mixin()`, `__call()`, the `$mixins` property, the five helper registrations at :104–118, `when()` and `templatePath()`. A theme that wrote `$theme->data()` writes `ntdst_data()`. `on()` and `filter()`: [NEEDS CLARIFICATION: keep `Theme::on()` / `filter()` (21 consumer readers, josworld `ViteAssets` mostly) as the chainable-config case of philosophy §5, or drop them as pass-throughs over `add_action`/`add_filter` — the drift reviewer's own punch list calls a pass-through method drift]. core-shape FR-12's trims apply in addition.
  Source: D1 — "logical and easy to understand for agents" (a `__call` surface cannot be read); `## Context`
- **FR-9:** `NTDST_Mailer`: [NEEDS CLARIFICATION: trim in place to the five members netdust-mail uses (`to()`, `cc()`, `bcc()`, `subject()`, `from()`, `message()`, `template()`, `attach()`, `send()`) and delete the rest, or move the whole class into stride's `netdust-mail` plugin where its only consumer lives and delete `services/Mailer.php` from core — philosophy §6.1 asks for several independent consumers and there is one]. Either way `queue()`, `toArray()`, `header()`, `ntdst_send_mail()`, `ntdst_notify()`, `ntdst_wrap_email_in_layout()`, the inline layout heredoc, the four `ntdst_mail_*`/`ntdst_email_*` filters and the two `ntdst_mail_*` hooks go.
  Source: D3 — `## Context` (one consumer, 0 readers of everything else)
...
- **FR-11:** The six `ntdst_model_{create,update,delete}_{before,after}` actions become `ntdst/model/{creating,created,updating,updated,deleting,deleted}` with the same arguments; daan `PressKitService` is renamed with them. After FR-5/FR-9 no `ntdst_`-prefixed hook remains in core; the `ntdst/*` convention in Bootstrap's header is the only one.
...
- **FR-12:** README's 5.0.0 section gains a "core-trim" migration table (removed symbol → what to write instead, one row each) covering every removal in FR-1…11, and a two-line "How to load core and your services" paragraph stating D2. `docs/philosophy.md` §4 drops the fail-open wart paragraph (it is fixed) and §1 cites FR-1. `ARCHITECTURE-INVARIANTS.md` gains INV-9 — *"A public symbol of core has at least one reader outside its own file, in core or in a D4 consumer; the zero-reader sweep is the mechanical check"* — and INV-10 — *"Core loads nothing by guessing: no directory scan, no source parsing, no path derived from a class name; a class is resolvable before `register()` sees it"* with check `grep -c "glob(\|file_get_contents(\|preg_match('/^\\\\s*namespace" core/Bootstrap.php` = 0.
  Source: core-shape FR-13/FR-16 pattern; D2
...
- **SC-2:** `grep -c "function_exists('ntdst_" api/*.php core/*.php admin/*.php services/*.php` = 0; `grep -n "require_once.*Logger" ntdst-core.php` appears on a lower line number than `require_once.*api/Data`.
...
- **SC-5:** `composer gate` exits 0 in core and, after their adapt commits, in daan (ProfileService, PressKitService), stride (AbstractRepository, GateReminderService, netdust-mail), josworld (meta accessor), todai — 5 green gates, 0 files changed outside the named consumers' files.
...
- stride's single `services.overrides` entry is the only reader of the config-override mechanism; the plan's ground-truth task decides FR-2's last sentence.
