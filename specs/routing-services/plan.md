...
4. **Server memory and CPU** — an anonymous caller posts a huge or deeply-nested JSON body to a write verb. **Mitigation:** NOT this class. WP reads the body and decodes it at depth 512 before any hook reachable here, so a route-level cap is theatre — and the size cap was bypassable via multipart, where php://input is empty. The controls are the web server's `client_max_body_size` / `post_max_size`, plus WP's own 512 depth bound. Recorded as an adoption checklist item, not a code mitigation. (withdrawn, was T05)
